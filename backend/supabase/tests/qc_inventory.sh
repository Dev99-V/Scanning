...
psql -c "insert into reference_stock (batch_id, stock_code, warehouse, bin, qty) values ('QCTEST8303','S1','01','cc',7);" > /dev/null || fail "seed lower-case reference"
psql -c "insert into scanned_data (batch_id, qty, bin, status) values ('QCTEST8303',7,'cc','pending');" > /dev/null || fail "seed lower-case scan"
[ "$(psql -c "select (recompute_scanned_statuses()->>'ok');")" = "true" ] || fail "recompute failed"
