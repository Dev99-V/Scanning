-- Migration 20260905050000:
-- Tạo RPC add_reference_stock: cho phép thêm dòng dữ liệu nguồn mới vào reference_stock,
-- tự động tính lại status cho các dòng scanned_data liên quan (từ not_in_reference sang ok/mismatch).
...
grant execute on function public.add_reference_stock(text, text, text, text, numeric, timestamptz, boolean) to anon, authenticated, service_role;
