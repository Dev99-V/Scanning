-- Migration 20260909090000: bịt khe race trong add_reference_stock (thẻ Thêm dữ liệu nguồn).
-- Triệu chứng: 2 người thêm cùng 1 Tag ID mới gần như cùng lúc → cả 2 SELECT đều
-- thấy "chưa có" → cả 2 INSERT → 1 người ăn lỗi unique constraint 23505 thô
-- (frontend chỉ hiện "Lỗi thêm dữ liệu..."), thay vì cảnh báo trùng đẹp
-- duplicate_batch_id kèm nút "Ghi đè cập nhật".
-- Cách sửa (đúng quyết định concurrency_strategy=db_row_lock_rpc): lấy
-- pg_advisory_xact_lock theo batch_id NGAY SAU khi trim batch_id, TRƯỚC khi
-- SELECT kiểm tra tồn tại — cùng mẫu với scan_submit/resolve_duplicate.
-- Dùng CHUNG namespace 'scan_submit:<batch>' với scan_submit/resolve_duplicate để
-- thêm-vs-quét cùng Tag cũng serialize (cả 2 đều đọc/ghi dòng reference_stock và
-- các dòng scanned_data liên quan), thứ tự ai trước ai sau luôn xác định, không
-- còn trạng thái dở dang giữa chừng. Không đổi params/returns (frontend giữ nguyên).
-- Lock là xact-scoped: tự nhả khi transaction commit/rollback, kể cả khi return sớm.
