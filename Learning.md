...
### [2026-09-21] Nạp file nguồn xong Bảng 1/Bảng 3 đối chiếu lệch im tới khi F5 + Bảng 3 không biết nguồn vừa đổi

- **Khu vực**: `App.tsx` + `ReferenceDataTable.tsx` + `useReferenceMap.ts` + mới `lib/inventoryCompare.detectSourceChange` + `InventoryScanModal.tsx` (Bảng 3)
- **Triệu chứng**: user hỏi check kỹ — nạp dữ liệu nguồn có update + highlight không, Bảng 3 có tự tra + cảnh báo khi nguồn update khác thời điểm trước không. Rà code thấy 2 lỗ hổng thật: (1) `import-reference` xóa-nạp lại toàn bảng (`delete().neq(batch_id,"")` rồi upsert từng chunk ~nghìn dòng) nhưng `App` không hề refetch `useReferenceMap` sau import (`ReferenceImportCard onImportSuccess` chỉ bump `refreshTrigger` nội bộ Bảng 2) → `byBatch` dùng cho Bảng 1 + Bảng 3 chỉ trông chờ realtime từng dòng, giữa import còn bị DELETE mass làm rỗng map (hiện "Ngoài nguồn" thoáng qua) rồi miss event → đối chiếu sai im tới khi F5; (2) Bảng 3 tính `compareInventoryRow` tại render nên tự tra lại theo nguồn mới, nhưng không có mốc "trước khi update" → không phân biệt được dòng vừa bị nguồn đổi với dòng đã lệch từ trước.
- **Nguyên nhân gốc** (đối chiếu code, không đoán): `App.tsx` destructure `useReferenceMap()` thiếu `refetch`; realtime `postgres_changes` từng dòng không đảm bảo đủ cho bulk delete+upsert nghìn dòng; Bảng 3 không snapshot giá trị nguồn.
- **Cách sửa**:
  1. `ReferenceDataTable` thêm prop optional `onReferenceImported` (gọi kèm `setRefreshTrigger` trong `onImportSuccess`); `App` lấy `refetchReference` từ `useReferenceMap` và truyền xuống → sau import reload full map, Bảng 1/Bảng 3 đúng ngay không cần F5.
  2. `inventoryCompare.ts` thêm pure `detectSourceChange(baseline, sys)` (đổi SL/Bin, bị xóa khỏi nguồn, mới thêm vào nguồn; `qty: NaN` = lúc chốt chưa có trong nguồn).
  3. `InventoryScanModal` chốt `baseline` lúc vừa mở modal; khi đang mở chỉ bổ sung Tag mới (không ghi đè mốc cũ); dòng bị nguồn đổi sau mốc → viền vàng + badge `🔄 Nguồn vừa đổi` + chi tiết cũ→mới + banner đếm Tag + nút `✓ Đã xem — chốt lại mốc`.
