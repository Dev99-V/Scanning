...
echo "--- CHECK 2/5: RLS authenticated đọc / anon không ---"
...
ANON_N="$(curl -s --max-time 30 "$API/rest/v1/scanned_data?select=id" -H "apikey: $ANON" -H "Authorization: Bearer $ANON" | jget "len(d)")"
[ "$ANON_N" = "0" ] || fail "anon sees scanned_data: $ANON_N"
pass "RLS"
